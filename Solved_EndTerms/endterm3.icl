...
//Start = count 3 [1,2,3,3,3,3,4]

removeOcc :: {Int} -> [Int]
removeOcc arr = filter ( \x = not(isEven(count x lst) )) lst 
where 
	lst = [ x \\ x <-: arr]
...
generate :: Int -> {Int}
generate n = { fst(x,y) \\ x <- [1..10] , y <- [1..x] }
//generate n = {n \\ n <- [1..10] , y <- [1..n]}
...
//Start =  flatten [ repeatn y y \\ y <- [1..10]]
...
*/
service :: {Product} -> Real
service arr = sum[p.price \\ p <-:arr]

//Start = service {fruits,vegetables}

aux :: Shop -> Real
aux sh = service(sh.products)

cheapestShop :: {Shop} -> (String,City)
cheapestShop arr = hd[ (x.shopName,x.location) \\ x <-: arr | (service x.products) == minList (map (\x = aux x ) lst)]
where 
	lst = [ x \\ x <-: arr]
...
pushBack [] a = [a]
pushBack [x] a = [x,a]
pushBack [x:xs] a = [x] ++ pushBack xs a
...
triples n = [ (a,b^3,c*3) \\ a <- [1..n] & b <- [1..n] & c <- [1..n]]
...
makeBinaryTree :: [a] -> Tree a
makeBinaryTree [] = Leaf
makeBinaryTree [x] = Node x Leaf Leaf
makeBinaryTree [x:xs] = Node (xs !! mid) (makeBinaryTree leftSubtree) (makeBinaryTree rightSubtree)
  where
    n = length xs + 1
    mid = n / 2
    leftSubtree = take mid xs
    rightSubtree = drop (mid + 1) xs
//Start = makeBinaryTree [1,2,3,4]
// bcollect :: ... -> [Int]
