...
aux1 :: [Cast] String -> Bool
aux1 lst str = or [ (c.name == str) \\ c <- lst ]

//Start = aux1 [cast4,cast8,cast7] "Sandra Bullock"
...
listByCast lst name = [ m.title \\ m <- lst | (aux1 m.casts name) ]
...
listByYearCountry lst yr coun = [x.title \\ x <- lst | (x.year == yr) && (x.country == coun) ]
