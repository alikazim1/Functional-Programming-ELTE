...
//given a matrix and return diagonal sum
diagSum :: [[Int]] -> Int
diagSum mtx 
...
largestMat :: {[[Int]]} -> [[Int]]
largestMat arr = hd[ x \\ x <- lst | diagSum x == maxList ( map (\x = diagSum x) lst)]
where 
	lst = [ x \\ x <-: arr]

//Start = diagSum Mat1
//Start = largestMat{Mat1}
...
assist :: (MyList a) -> String | toString a
assist Empty = ""
assist (Elem x Empty) = toString x
assist (Elem val next) = toString val +++ "," +++ assist next 

instance toString (MyList a) | toString a
where 
	toString lst = "[" +++ assist lst +++ "]"




//Start = [toString x \\ x <- [1,2,3]] 
...
firstAndMiddle Leaf = []
firstAndMiddle (Node (FirstName str) l r) = [str] ++ firstAndMiddle l ++ firstAndMiddle r
firstAndMiddle (Node (MiddleName str) l r) = [str] ++ firstAndMiddle l ++ firstAndMiddle r
firstAndMiddle (Node (LastName str) l r) = firstAndMiddle l ++ firstAndMiddle r
...
where 
	toInt (A a) = length [ x \\ x <-: toString a]
	//toInt (A a) = sum [ 1 \\ x <-: (toString a) ]
	toInt (B b) = 1
	
findWhich :: [(OneOf String Char)] -> Int
findWhich lst = sum[toInt x  \\ x <- lst ]

//Start = toIn "hello"
//Start = length [ x \\ x <-: "Hello"]
