...
isUnique tup 
| (fst3 tup <> snd3 tup) && (snd3 tup <> thd3 tup) && (fst3 tup <> thd3 tup) = True
= False

addd :: (Int,Int,Int) -> Int
addd tup = abs(fst3 tup + snd3 tup + thd3 tup)

isSmallest :: {(Int,Int,Int)} -> (Int,Int,Int)
isSmallest arr = hd[ (a,b,c) \\ (a,b,c) <- lst | isUnique (a,b,c) && addd(a,b,c) == minList(map (\x = addd x) lst )  ]
where lst = [ (a,b,c) \\ (a,b,c) <-: arr]
//Start = isUnique (2,2,3)
//abs
//Start = minList(map (\x = addd x) lst )
//Start = abs (1+3-6)
//unique :: [(Int,Int,Int)] -> [(Int,Int,Int)]

//Start = isSmallest {(-1,3,2), (-3,-1,-2)}

alterArray :: {(Int, Int, Int)} -> {(Int, Int, Int)}
alterArray arr = { (a * fst3 (tup),b * (snd3 tup),c * (thd3 tup)) \\ (a,b,c) <-:arr }
where 
	tup =  isSmallest arr
...
date :: (EngagedPerson, EngagedPerson) -> Int
date tup = (fst tup).yearOfMarriage

//Start = date (Leon, Ada)

compare :: [(EngagedPerson, EngagedPerson)] -> Int
compare lst = minList (map (\x = date x) lst )

ff :: String -> [Char]
ff str = [ ch \\ ch <-: str]

gg :: String -> String
gg str =toString(tl(dropWhile(\x = x <> ' ') (ff str) ))

//Start = gg "abc def"
//Start = compare [ (Leon, Ada), (Mike, Edna)]
getOldestMarriage :: {(EngagedPerson, EngagedPerson)} -> String
getOldestMarriage arr = gg(hd[ fst(a,b) \\ (a,b) <-: arr | a.yearOfMarriage == compare lst]).fullName
where 
	lst = [ (a,b) \\ (a,b) <-: arr]
//Start :: [EngagedPerson]
//arr :: {(EngagedPerson,EngagedPerson)}
//arr = { (Leon, Ada), (Mike, Edna)}
//Start = [ fst (a,b) \\ (a,b) <-: arr  ]// | a.yearOfMarriage == compare lst]
//where 
//	lst = [ x \\ x <-: { (Leon, Ada), (Mike, Edna)}]
...
neighbours lst = [lst !! (((ColorID a) + 2) rem 7) \\ a <- lst]
